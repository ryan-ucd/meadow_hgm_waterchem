...
DT::datatable(mod.hgm %>% dplyr::distinct(PLOTNAME2) %>% select(UCDID,PLOTNAME2, hgm_type, hgm_class_comb), 
              rownames = TRUE,
              class="compact row-border stripe",
              caption=htmltools::tags$caption(
                style = 'caption-side: bottom; text-align: center;',
...
save(dff, dfVI, dfWI, file = "./data_output/model_datasets_AUG.rda") 
...
# Model Datasets
load("./data_output/model_datasets_AUG.rda")
...
m1a <- map2stan(
  alist(
    ndvi ~ dnorm(mu, sigma), 
    mu <- a + a_mdwID[mdwID] + bwyi*wyi + brip*rip +
      bsub*sub + bds*ds + 
      bdep*dep + blacfr*lacfr,
    c(bwyi, rip, sub, ds, dep, lacfr) ~ dnorm(0,10),
    a ~ dnorm(0,10),
