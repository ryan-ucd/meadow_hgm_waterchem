...
ndvi<-dff %>% filter(index=="NDVI") %>% 
  select(UCDID, name_id, mean, mean_s, SAC, wtyr_typ, wtyr_seq, wtyrlag1,
...
m1a <- map2stan(
  alist(
    ...
    mu <- a + a_wtyr_typ[wtyr_typ] + bhgm*hgm + bclay*clay + 
      bflowslope*flowslope + belev*elev + 
      bkf*kf + bedge*edge + bcatch*catcharea,
    a ~ dnorm(0,10),
    a_wtyr_typ[wtyr_typ] ~ dnorm(0,sigma),
    sigma ~ cauchy(0,2),
    c(bhgm, bclay, bflowslope, belev, bkf, bedge, bcatch) ~ dnorm(0,10)
  ),
  data=dlist, chains = 2, warmup = 2000, iter=5000, cores = 2)
