...
# add month category for aggregating
dff <- mod.hgm %>% mutate(month=month(date)) %>%
...
# add WY column for modeling (combines both SAC and SJ based on location)
dff$dwr_name <- as.factor(dff$dwr_name)
dff$wy_index <- ifelse(dff$dwr_name == "SAC", dff$SAC_Index, dff$SJ_Index)
summary(dff)
...
dff$wy_indx<-(dff$wy_index - mean(dff$wy_index)) / sd(dff$wy_index)
...
ndvi<-dff %>%  
  select(UCDID, WY, month, mdw_id, mean, median, wy_indx,
         hgm_class_comb, lat_s, edgecomplex_s:flowrange_s)
summary(ndvi)

```

```{r mod1a, eval=F, echo=T}
...
m1a <- map2stan(
  alist(
    ...
    mu <- a + a_mdwID[mdwID] + bwyi*wyi + bhgm*hgm + 
      blat*lat + belev*elev + bclay*clay + bkf*kf,
    a ~ dnorm(0,10),
    #a_wyi[wyi] ~ dnorm(0,sigma),
    a_mdwID[mdwID] ~ dnorm(0,sigma),
    sigma ~ cauchy(0,2),
    c(blat, belev, bclay, bkf, bwyi) ~ dnorm(0,10),
    bhgm ~ dnorm(3,10)
  ),
  data=dlist, chains = 2, iter=5000, cores=3)
...
m1b <- map2stan(
  alist(
    ndvi_mean ~ dnorm(mu, sigma), 
    mu <- a + a_hgm[hgm] + bwyi*wyi + bElwy*elev*wyi +
      belev*elev + bcatch*catcharea + bFslope*flowslope,
    a ~ dnorm(0,10),
    #a_wyi[wyi] ~ dnorm(0,sigma),
    a_hgm[hgm] ~ dnorm(0,sigma),
    sigma ~ cauchy(0,2),
    bElwy ~ dnorm(0,10),
    c(belev, bcatch, bFslope, bwyi) ~ dnorm(0,10)
  ),
  data=dlist, chains = 2, iter=5000, cores=3)
...
m1c <- map2stan(
  alist(
    ndvi_mean ~ dnorm(mu, sigma), 
    mu <- a + a_mdwID[mdwID] + bwyi*wyi + bhgm*hgm + 
      bWYhgm*wyi*hgm + belev*elev + 
      bcatch*catcharea + bFslope*flowslope,
    a ~ dnorm(0,10),
    #a_hgm[hgm] ~ dnorm(0,sigma),
    #a_wyi[wyi] ~ dnorm(0,sigma),
    a_mdwID[mdwID] ~ dnorm(0,sigma),
    sigma ~ cauchy(0,2),
    bhgm ~ dnorm(0,10),
    c(belev, bcatch, bFslope, bwyi, bWYhgm) ~ dnorm(0,10)
  ),
  data=dlist, chains = 2, iter=5000, cores=3)
...
mtext("Varying effects model m1c")
postcheck(m1b)

save(m1a,m1b, m1c, file = "../data_output/models_1a-1c.rda")
