...
plot(precis(m1a))
plot(precis(m2a))
mtext("Mdw ID Intercept HGM Models", at = c(-0.2), line = 0.5, font=3)
par(mfrow=c(1,1)) # clear the plots
#dev.off()
...
# df1a
# ggplot() + geom_pointrange(data=df1a, aes(y=Mean, x=vars, ymax=upper89, ymin=lower89, color=vars), fatten = 4, pch=21, show.legend = F) + coord_flip() + xlab("Model Variables") +
#   ylab("Model Variance")
# 
## EXTRACT POSTERIORS
...
```{r mod_multi_intercept_hgm_m1b2b, eval=F, echo=T}
...
# SAVE THEM
save(m1b, m2b, file = "./data_output/models/models_1b_2b_revised.rda")
...
```{r mod1c2c_mdw_vars, eval=F, echo=T} 

m1c <- map2stan(
  alist(
    ndvi ~ dnorm(mu, sigma), 
    mu <- a + a_WY[WY] + bhgm*hgm + bsub*sub + bds*ds +
      belev*elev + bcatch*catcharea + 
      bFslope*flowslope + blat*lat + bkf*kf + bclay*clay + bshape*shape,
    a_WY[WY] ~ dnorm(0, sigma),
    a ~ dnorm(0,10),
    c(a, belev, bhgm, bsub, bds, bcatch, bFslope, 
      blat, bkf, bclay, bshape) ~ dnorm(0,10),
    sigma ~ cauchy(0,2)
  ),
  data=dlist1, chains=2, iter=5000, cores=2)

summary(m1c) # see all 
plot(m1c) # see trace plots
par(mfrow=c(1,1))
precis(m1c, depth=2)
plot(precis(m1c)) # see basic results with fixed effects
plot(precis(m1c, depth=2)) # see full results for each meadow
...
#save(m1c, file = "../data_output/models/models_1c_revised.rda")
#load("../data_output/models/models_1c_revised.rda")

m1d <- map2stan(
  alist(
    ndvi ~ dnorm(mu, sigma), 
    mu <- a + a_mdwID[mdwID] + bwyi*wyi + bsub*sub + bds*ds + 
      belev*elev + bcatch*catcharea + bFslope*flowslope + blat*lat + 
      bkf*kf + bclay*clay + bshape*shape,
    a ~ dnorm(0,10),
    a_mdwID[mdwID] ~ dnorm(11,sigma),
    c(bwyi, bsub, bds, belev, bcatch, 
      bFslope, blat, bkf, bclay, bshape) ~ dnorm(0,10),
    sigma ~ cauchy(0,2)
  ),
  data=dlist1, chains = 2, iter=5000, cores=2)


save(m1c,m2c, file = "../data_output/models_1c_2c_revised.rda")
...
plot(gg_vimp(rfWI.2, nvar=10)) + ylab("NDWI Variable Importance") #+
  #scale_y_continuous(labels = scales::percent_format(), limits=c(0,1))

plot(gg_vimp(rfVI.2, nvar = 10))+ ylab("NDVI Variable Importance")
