...
# WITH PALETTE
ggSnw <- gg + geom_point(data=snow.hgm.df[snow.hgm.df$Year==2013,], 
...
```{r make a loop, eval=F, echo=T}

# i<-2011
# table(snow.hgm.df$Year)

for (i in c(1930:max(snow.hgm.df$Year))){
...
ndvi_map <- function(Yr){
  ...
  gg + geom_point(data=ndvi.df[ndvi.df$WY==Yr,], 
                  ...
                  show.legend=F, pch=21, size=4.4, color="gray30")+ 
    theme_bw() + ylab("Latitude") + xlab("Longitude") +
    theme(axis.text.x = element_text(angle = 60, vjust=0.15, size=8),
          legend.position=c(1,1),legend.justification=c(1,1),
          legend.direction="vertical",legend.text=element_text(size=8),
          ...
          legend.box.just = c("top"), 
          legend.background = element_rect(fill=alpha('white', 0.6), colour = "gray30")) +
    # scale_fill_gradientn("NDVI (Aug)",
                         # colours=brewer.pal(8,name="RdYlGn"), 
                         # limits=range(breaks), breaks=breaks)+
...
system(command= "convert ~/Documents/github/meadow_hgm_waterchem/fig_output/cdec/*.png -delay 80 -loop 0 cdec_snow_IM.gif")
